...
unsigned char scan_BT()
{
  ...
  {      
    ...
    {
      tam1 = 1;
      for(i=0;i<60;i++) // kiem tra BT nhan giu 6s
      {  
        delay_ms(100);
        if(input(BTON1)==1) 
        {
            if(i < 30) // nhan it hon 3s, tra ve che do on1
              return 1;
            else if(i >= 30) // nhan lau hon 3s, tra ve che do on2             
               return 2;
        }
        if(i >= 30)
        {
      ...
      return 11; // nhan lau hon 6s, tra ve che do hoc lenh
    }
  }
